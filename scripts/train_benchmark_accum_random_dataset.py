...
def main():
    ...
    # torch.backends.cuda.enable_flash_sdp(enabled=False) # cuda only support flash atten dim <= 128; but pretrained has 512
    ...
    device = get_current_device()  # device cuda:0
    dtype = to_torch_dtype(cfg.dtype)
    # torch.backends.cuda.flash_sdp_enabled() # backends have no attr cuda; if rm cuda, then torch.backends has 
    ...
    # seed = torch.randint(low=1000, high=9999, size=(1,)).cuda()
