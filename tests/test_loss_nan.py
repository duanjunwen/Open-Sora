...
from transformers.models.t5.modeling_t5 import T5Attention,  T5LayerSelfAttention, T5LayerNorm, T5LayerFF, T5DenseGatedActDense, T5Config
...
def test_T5LayerSelfAttention():
    ...
    print(out)  # (hidden_states,) + attention_output[1:]
    out.mean().backward()
